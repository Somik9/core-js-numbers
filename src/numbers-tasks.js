...
function getFloatOnString(str) {
  ...
  return String(num) === str.trim().slice(0, String(num).length) ? num : NaN;
}
...
function getIntegerOnString(str, base) {
  const integerPart = Number.parseInt(str, base);
  if (Number.isNaN(integerPart)) {
    return NaN;
  }
  const reconstructedStr = integerPart.toString(base);
  const trimmedStr = str.trim().toLowerCase();
  return trimmedStr.startsWith(reconstructedStr) ? integerPart : NaN;
}
